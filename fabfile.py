...
def setup():
    ...
    require.deb.packages([
        "i3", "lightdm", "xfce4-terminal", "build-essential", "python-pip",
        "unzip"])
    ...
    run("pip3 install tmuxp powerline-shell pwdman hostscli")
    ...
    with cd(tmp_dir):
        ...
        for repo, repo_name in zip(repos, repo_names):
            wget("%s/%s/archive/%s.zip -O %s.zip" % (gh, repo, branch, repo_name))
